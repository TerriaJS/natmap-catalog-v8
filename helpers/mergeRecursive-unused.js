...
function mergeRecursive(catalogMembers) {
  ...
  catalogMembers.forEach((m) => {
    ...
    if (matchIndex >= 0 && existingM.members == null) {
      console.warn(
        "Merging 2 members, the first of which isn't a catalog group. This is probably not intended"
      );
      console.warn(
        `Members are: Member(id:${m.id},name:${m.name},nameInCatalog:${m.nameInCatalog}) & Member(id:${existingM.id},name:${existingM.name},nameInCatalog:${existingM.nameInCatalog})`
      );
    } else {
      // For a catalog group, concat members and merge properties
      if (existingM.members != null) {
        updatedM.members = mergeRecursive(
          [].concat(existingM.members, m.members || [])
        );
      }
      if (matchIndex >= 0) {
